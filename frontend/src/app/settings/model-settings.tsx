...
function ServiceEditor({
  ...
  async function save() {
    setBusy("save");
    try {
      const result = await write<Provider>(
        service ? `/model-services/${service.id}` : "/model-services",
        { name, base_url: url, ...(secret ? { secret } : {}) },
        service ? "PUT" : "POST",
      );
      setSecret("");
      await refresh();
  ...
  async function discover() {
    ...
    setBusy("models");
    try {
      const result = await write<{ models: string[] }>(
        `/model-services/${service.id}/models`,
        {},
      );
  ...
  async function saveFilter() {
    ...
    setBusy("filter");
    try {
      await write(`/model-services/${service.id}/filter`, { keywords }, "PUT");
      await refresh();
      toast.success("模型过滤已保存");
    } catch (e) {
      toast.error((e as Error).message);
    } finally {
      setBusy("");
  ...
  const filteredModels = models.filter(
    (model) =>
      !terms.length || terms.some((term) => model.toLowerCase().includes(term)),
  );
    ...
    <section className="service-editor">
      <div className="service-fields">
        ...
        <div>
          ...
          <Input
            ...
            placeholder={
              service?.configured ? "已配置，留空保留现有密钥" : "输入密钥"
            }
          />
        </div>
        <section className="model-catalog" aria-label="模型列表">
          <div className="model-catalog-heading">
            ...
            <InfoHint>
              按模型名称过滤，多个关键词用逗号分隔。应用后，下方模型选择仅显示匹配结果。
            </InfoHint>
            <Button
              variant="ghost"
              size="icon"
          ...
          {service && (
            <div className="model-catalog-toolbar">
              ...
              <Input
                ...
                placeholder="过滤模型，如 qwen, gpt"
              />
              <Button
                variant="outline"
                disabled={!!busy || !filterChanged}
                onClick={() => void saveFilter()}
              >
                {busy === "filter" ? "保存中…" : "应用过滤"}
              </Button>
            </div>
          )}
          <div className="model-catalog-list" key={keywords}>
            {filteredModels.length ? (
              filteredModels.map((model) => (
                <div className="model-catalog-item" key={model}>
                  <OverflowText text={model} />
                </div>
              ))
            ) : (
              <div className="model-catalog-empty">
                {models.length ? "没有匹配的模型" : "尚未获取模型"}
              </div>
            )}
          </div>
        </section>
      ...
      {!locked && (
        <div className="service-actions">
          <Button
            disabled={
              !!busy || !name.trim() || !url.trim() || (!service && !secret)
            }
            onClick={() => void save()}
          >
            {busy === "save" ? "保存中…" : "保存服务"}
...
function ModelChoices({ settings }: { settings: Settings }) {
  ...
  const [choices, setChoices] = useState<Record<string, Choice>>({
    default: initial,
    report: settings.llm_routes?.report || initial,
    assumptions: settings.llm_routes?.assumptions || initial,
  });
  const [busy, setBusy] = useState("");
  const refresh = useRefresh("settings");
  async function save() {
      ...
      await write(
        "/settings",
        {
          ...
          llm_routes: {
            ...
            assumptions: choices.assumptions,
          },
        },
        "PUT",
      );
      await refresh();
      toast.success("模型选择已保存");
    } catch (e) {
      toast.error((e as Error).message);
    } finally {
      setBusy("");
  ...
  async function test(key: string) {
    setBusy(key);
    try {
      await write("/settings/test", { ...choices[key], data_mode: "auto" });
          ...
          <TableBody>
            {[
              ...
              const choice = choices[key],
                service = settings.providers.find(
                  (p) => p.id === choice.provider,
                );
              return (
                <TableRow key={key}>
                  ...
                  <TableCell>
                    <Select
                      value={
                        service?.selectable_models.includes(choice.model)
                          ? choice.model
                          : ""
                      }
                      onValueChange={(model) =>
                        setChoices({ ...choices, [key]: { ...choice, model } })
                      }
                      disabled={!service?.selectable_models.length}
                    >
                      <SelectTrigger
                        className="w-full"
                        aria-label={`${label}模型`}
                      >
                        <SelectValue
                          placeholder={
                            service?.selectable_models.length
                              ? "请选择模型"
                              : "无匹配模型"
                          }
                        />
                      ...
                      <SelectContent>
                        {service?.selectable_models.map((model) => (
            ...
            Object.values(choices).some(
              (c) =>
                !settings.providers
                  ...
                  ?.selectable_models.includes(c.model),
            )
          }
          onClick={() => void save()}
        >
  ...
  return (
    <>
      <Card>
        ...
        <CardContent className="service-layout">
          <nav className="service-navigation" aria-label="模型服务">
            {settings.providers.map((p) => (
              <Button
              ...
              >
                ...
                <span className="model-service-name">
                  {p.name}
                  <small>{p.configured ? "已配置" : "未配置"}</small>
                </span>
