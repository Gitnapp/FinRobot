...
export function MacroDetailPage() {
  ...
  const range =
    selection?.metric === metric ? selection.range : seriesWindow(latest?.date);
...
function monthRange(month: string) {
  const [year, value] = month.split("-").map(Number);
  return {
    from: new Date(Date.UTC(year, value - 1, 0)).toISOString().slice(0, 10),
    to: new Date(Date.UTC(year, value, 1)).toISOString().slice(0, 10),
  };
}
export function CalendarPage() {
  ...
  const q = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam, signal }) => {
      ...
      const snapshot = await api<Snapshot<Calendar>>(
    ...
    getPreviousPageParam: (first) =>
      first.month > "1900-01" ? shiftMonth(first.month, -1) : undefined,
    getNextPageParam: (last) =>
      last.month < "9999-12" ? shiftMonth(last.month, 1) : undefined,
  });
  const futureAt = q.data?.pages
    .flatMap((p) => p.snapshot.data?.events || [])
    .filter((e) => new Date(e.at).getTime() >= Date.now())
    .sort((a, b) => a.at.localeCompare(b.at))[0]?.at;
  useLayoutEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      ...
      const boundary = rows.findIndex((row) =>
        row.classList.contains("future-start"),
      );
  ...
  useEffect(() => {
    if (
      ...
      q.isFetching ||
      !q.hasPreviousPage ||
      !q.data?.pages[0]?.snapshot.data?.events.length
    )
      return;
    const observer = new IntersectionObserver(
      (entries) => {
        if (!entries[0].isIntersecting) return;
        const element = timeline.current?.querySelector(
  ...
  const canAutoLoad =
    Boolean(tail?.snapshot.data?.events.length) && !q.isFetching;
  useEffect(() => {
      ...
      <div className="calendar-month-picker">
        ...
        <Button
          ...
          onClick={() => {
            ...
            setMonth(new Date().toLocaleDateString("sv-SE").slice(0, 7));
        ...
        <InfoHint>
          浅底色表示过去的事件，分界线后为即将到来的事件。选择月份快速跳转，上下滚动连续查看相邻月份。未公布事件或数据暂不可用时保留真实空状态，不补造事件。
        </InfoHint>
      </div>
      <div
        ...
        <div ref={timeline} className="calendar-months">
          {q.data?.pages.map(({ month: period, snapshot }) => {
            const events = (snapshot.data?.events || [])
              .filter((e) =>
                new Date(e.at).toLocaleDateString("sv-SE").startsWith(period),
              )
              .sort((a, b) => a.at.localeCompare(b.at));
            return (
              <section key={period} className="calendar-month-section">
                <h2>{period.replace("-", " 年 ")} 月</h2>
                {events.map((e, i) => (
                  <div
                    className={`macro-event${new Date(e.at).getTime() < Date.now() ? " is-past" : e.at === futureAt ? " future-start" : ""}`}
                    key={e.at + e.title + i}
                  >
                    <time>
                    ...
                    <div>
                      ...
                      <small>
                        ...
                        {String(e.previous ?? "—")}　预期{" "}
                        {String(e.forecast ?? "—")}　实际{" "}
                        {e.actual_status === "unverified" ? (
                          <InfoLabel label="待核实">
                            零值未经有效公布状态确认，暂不作为实际值。
                          </InfoLabel>
                ...
                {!events.length && snapshot.state !== "pending" && (
