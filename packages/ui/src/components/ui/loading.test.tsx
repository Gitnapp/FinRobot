...
it("merges nested pending states into one indicator and removes it when ready", async () => {
  ...
  await act(async () => {
    root.render(<LoadingScope active><LoadingState/><LoadingScope active><LoadingState/></LoadingScope></LoadingScope>);
  });
  expect(host.querySelectorAll('[role="status"]')).toHaveLength(1);
  expect(host.textContent).toBe("");
  await act(async () => root.render(<LoadingScope><span>Ready</span></LoadingScope>));
