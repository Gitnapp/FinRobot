...
export function LoadingScope({
  ...
  return (
    <LoadingContext.Provider value={value}>
      <div className="rui-loading-scope">{children}
      {!parent && (active || count > 0) && (
        <div
          role="status"
          aria-label="正在加载"
          className="rui-loading-scope-indicator"
        >
          <MathCurveLoader/>
        </div>
      )}
...
export function LoadingState({ className, inline = false, mode = "initial", message = "首次加载可能需要一些时间，请稍候。" }: { className?: string; inline?: boolean; mode?: "initial" | "refresh"; message?: string }) {
  const register = useContext(LoadingContext);
  const [slow,setSlow] = useState(false);
  useEffect(() => inline ? undefined : register?.(), [register, inline]);
  useEffect(()=>{if(mode!=="initial")return;const timer=setTimeout(()=>setSlow(true),2000);return ()=>clearTimeout(timer);},[mode]);
  if (register && !inline) return null;
  return <div role="status" aria-label="正在加载" className={cn("rui-loading-state",className)}>
    <div className="rui-loading-center"><MathCurveLoader/></div>
