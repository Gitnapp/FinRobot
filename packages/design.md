...
- 折叠态图标轨（`48px`）的入口高亮固定 `size-8` 并水平居中。触屏命中区只在展开态用
  36px 着色面与上下各 2px 的透明命中区，导航项间留 4px，**不得把 44px 高度带进折叠态**，否则高亮退化成长方形。
...
手机按钮保持 36px 可视高度、14px 文字，图标按钮保持正方形。44px 触摸范围由透明命中区域扩展，不得用通用 button min-height 把信息图标或小控件撑大。输入框仍保持 16px 字体以避免 iOS 聚焦缩放。

### 表单与弹窗密度

弹窗标题左对齐，16px/24px；内边距手机 20px、桌面 24px，内容区间距 20px。表单标签 13px/20px、次要文字色，标签与输入框间距 8px。输入框高 40px，水平内边距 12px，手机文字保持 16px；焦点环 2px、低透明度。Select 高 36px，浅灰底、14px 字号，文字左侧和箭头右侧均留 12px。长提示文字自然换行，不使用均衡换行。辅助操作使用有可访问名称的图标按钮；相关管理动作合并为菜单，业务操作顺序由应用决定。
...
卡片统一通过 Card / CardHeader / CardContent / CardFooter 组织：内边距 20px，标题区到内容区 16px，内容内项目间距 12px。禁止子标题 margin 与父容器 padding 叠加撑高空白；更新状态放在 CardAction，与标题同一行。
...
文字旁的说明图标统一使用 InfoLabel，禁止直接在行内文字后拼接 InfoHint。组合继承上下文字号与行高，文字和图标垂直居中、间距4px；独立卡片说明图标在 CardAction 内与标题垂直居中，不使用偏移量修补基线。

导航字重层级：品牌14px/500；普通一级导航14px/400，选中项500；当前位置用次要文字色和常规字重。章节目录普通项12px/400、当前项14px/600，仅用放大字号和加粗强调；禁止选中底色、侧边线、内阴影或色块。固定行高，保留轻微字号和颜色过渡。
...
重复数据卡片使用弹性等宽网格与统一行高；宽度由容器决定，同一视图的所有行保持等高。禁止固定像素宽度导致窗口留空或窄屏横向溢出；内容与底部操作区保持固定关系。


证券身份展示：公司名称为主标题，交易所证券目录提供的股票代码为次标题。行情返回名称为统一来源，不以手写简称覆盖；缺少可核实名称时不得虚构。手机研报目录使用可点击的当前章节入口与可滚动章节面板，触摸操作不能依赖悬停；选择章节后关闭面板并定位正文。

历史日程使用固定条数分页，不使用不断追加记录的“更多”；分页不累积卡片高度。

所有事件日历提供统一日期范围选择器，筛选不能冒充服务端尚未取得的历史。宏观指标可进入详情，图表须提供可访问的日期/数值读数及悬停交互，移动端直接触摸图表查看读数，不额外添加无明确用途的滑条。同比/环比必须来自正确口径，利率差使用百分点，不把增速再次当作指数计算。

日期筛选优先使用一键常用范围，最多五个入口；自定义范围在同一日历中点击起止两天即生效，不展示两组原生日期输入，不增加应用确认。

顶部面包屑必须映射当前路由层级：父级为可点击链接，当前页不可点击并设置 aria-current；桌面与手机均可返回父级，长标题省略不挤掉父级入口。名称复用页面已取得的数据，不为导航额外请求接口。

跟踪管理统一放在标的详情三点菜单：更新频率为独立标注组，暂停/恢复和移除集中于同一入口。列表卡片只展示状态和查看入口。成员身份不依赖财报权限；缺财报不应使跟踪操作消失。

缓存到期不等于数据源失败。已有有效内容的正常后台刷新不显示待更新徽标或加载动画；失败时才显示待更新与最近有效时间。刷新任务由后端限并发队列完成，不依赖用户再次打开页面或多次轮询才能启动剩余任务。

同类公司卡片的财务区域固定展示营业收入、EBITDA、毛利率，字段顺序和位置一致。缺失值显示 — 并在说明中标明缺口，禁止以开盘价、最高价、最低价等无关指标替换整组财务字段。

所有市场的标的详情共用同一组件结构与字段定义，不因供应商或财报权限切换精简页面。数据状态只影响值、来源说明和操作是否可用，不隐藏整个财务、公告、情绪、估值或模型栏目。保留原币种与报告期，未取得的值用 —，不从美元模型推造本币数据。
